...
@export var duracion_aturdimiento_post_embestida: float = 0.65
...
var _activo_en_arena: bool = false
...
func activar_sello() -> void:
	if _derrotado or not _activo_en_arena:
		return
...
func establecer_activo_en_arena(activo: bool) -> void:
	var nuevo_estado := activo and not _derrotado
	if _activo_en_arena == nuevo_estado:
		return

	_activo_en_arena = nuevo_estado
...
func _procesar_embestida(delta: float) -> void:
	...
	if _temporizador_estado == 0.0:
		_estado_jefe = EstadoJefe.ATURDIDO
		_temporizador_estado = duracion_aturdimiento_post_embestida
		visual.scale = _escala_visual_original
		visual.modulate = color_base.lerp(color_debilitado, clampf(float(_sellos_activados) / float(max(sellos_necesarios, 1)), 0.0, 1.0))
...
func establecer_congelado(congelado: bool) -> void:
	super.establecer_congelado(congelado)
	area_ataque.monitoring = _activo_en_arena and not _congelado and not _derrotado
