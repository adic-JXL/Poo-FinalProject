...
@export var multiplicador_velocidad_por_fase: float = 0.22
@export var enfriamiento_embestida_base: float = 2.6
@export var duracion_carga_base: float = 0.62
@export var duracion_embestida: float = 0.30
@export var multiplicador_embestida: float = 2.9
@export var duracion_aturdimiento_sello: float = 0.65
...
func activar_sello() -> void:
	...
	_enfriamiento_embestida = max(0.8, enfriamiento_embestida_base - (_fase_actual * 0.35))
...
func _procesar_embestida(delta: float) -> void:
	...
	if _temporizador_estado == 0.0:
		...
		_enfriamiento_embestida = max(0.75, enfriamiento_embestida_base - (_fase_actual * 0.45))
		_actualizar_linea_carga(false)
...
func _empezar_carga() -> void:
	...
	_temporizador_estado = max(0.24, duracion_carga_base - (_fase_actual * 0.09))
	_actualizar_linea_carga(true)
...
func _actualizar_linea_carga(visible: bool) -> void:
	...
	linea_carga.points = PackedVector2Array([Vector2.ZERO, _direccion_embestida * 132.0])
